...
#include "ros2_art_test/action/navigate.hpp"
#include "geometry_msgs/msg/point.hpp"

typedef ros2_art_test::action::Navigate NavigateAction;
...
class NavigateActionServerNode : public rclcpp::Node
{
 ...
 NavigateAction::Goal> goal)
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
      ...
      rclcpp::Rate loop_rate(100); // Rate in Hz to calculate distance, publish feedback

      // auto a = goal->goal_point.x;
      // // ros2_art_test::action::Navigate_Goal().goal_point.x

      feedback->distance_to_point = DIST_THRESHOLD; // Init value to trigger while loop

double step = 0.01; // Adjust this according to your requirements
  ...
  while(feedback->distance_to_point >= DIST_THRESHOLD)
  {
    ...
    // Change robot's x position here
    if (this->robot_position_.x < goal->goal_point.x)
    {
      // Move towards goal on x-axis
      this->robot_position_.x += step ;
    }
    else if (this->robot_position_.x > goal->goal_point.x)
    {
      // Move towards goal on x-axis
      this->robot_position_.x -= step;
    }

    // Change robot's y position here
    if (this->robot_position_.y < goal->goal_point.y)
    {
      // Move towards goal on y-axis
      this->robot_position_.y += step ;
    }
    else if (this->robot_position_.y > goal->goal_point.y)
    {
      // Move towards goal on y-axis
      this->robot_position_.y -= step ;
    }

    // Change robot's z position here
    if (this->robot_position_.z < goal->goal_point.z)
    {
      // Move towards goal on z-axis
      this->robot_position_.z += step ;
    }
    else if (this->robot_position_.z > goal->goal_point.z)
    {
      // Move towards goal on z-axis
      this->robot_position_.z -= step ;
    }
